...
def load_model(model_path):
    model = AutoModelForCausalLM.from_pretrained(model_name, cache_dir='./', token=os.getenv('HF_TOKEN'), trust_remote_code=True)
...
# Load tokenizer
tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir='./', token=os.getenv('HF_TOKEN'), trust_remote_code=True)
...
def process_models(model_list):
    ...
    # Load tokenizer once
    model_name = "Qwen/Qwen2-0.5B"
    tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir='./', token=os.getenv('HF_TOKEN'))
    tokenizer.pad_token = tokenizer.eos_token
