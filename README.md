# **Auraflow: Accelerators for AI on Edge. Reimagined.**

This repository contains the code, methods, and scripts for implementing **Auraflow Quantization** and **Lottery Ticket Hypothesis (LTH)** techniques for optimizing neural networks. The repository focuses on various quantization algorithms, model evaluations, and fine-tuning techniques to minimize perplexity and stabilize activations.

---

## **What is Auraflow?**  

**Auraflow** is a custom quantization algorithm that operates with a **scalable precision format**. Unlike traditional floating-point systems (IEEE-754), Auraflow removes the mantissa entirely and focuses solely on the **exponent** for precision representation.  

### **Key Features**:  
1. **Sign-Exponent Representation**:  
   - Auraflow (AFx) uses `1 bit` for the **sign** and allocates the remaining `x-1 bits` for the **exponent**.  
   - For instance, in **AF16**:  
     - 1 bit → Sign  
     - 15 bits → Exponent  
...
3. **Bit-width Flexibility**:  
   - Auraflow supports variable precision formats, scaling between **3-bit and 16-bit**:  
     - Lower precision (e.g., **AF4**) → Faster computation, reduced model size.  
     - Higher precision (e.g., **AF16**) → Improved accuracy while maintaining efficient quantization.

4. **Gradient and Activation Capping**:  
   - To stabilize the training process, gradients and activations are **capped** at -1 and +1.

### **Advantages of Auraflow**:  
...
**Conversion FP32 - AF(4-16)**

A standard 32-bit floating-point number is converted into a custom auraflow representation with a variable-sized mantissa.
...
**WASQ** stands for **Weight and Activation Auraflow Quantization**. It is a **hybrid quantization framework** that leverages Auraflow precision to optimize both model weights and activations.

### **Key Characteristics of WASQ**:  
1. **Weight Quantization**:  
   - Model weights are converted to **Auraflow precision** (AFx) without requiring complex computations like mantissa adjustments.  
...
1. **Quant_Dequant.ipynb**  
   Contains the implementation of basic Auraflow quantization and dequantization functions.

2. **sf16quant.ipynb**  
   Builds on Auraflow quantization functions, specifically for **AF16 precision**.
...
1. **Auraflow Precision**: Custom precision without mantissa, clamped within `[-1, 1]` for stability.  
...
```bash
git clone https://github.com/YOUR_USERNAME/auraflow
cd auraflow
...
Atreides is an ASIC accelerator designed specifically for Auraflow-based inference. We redesigned the systolic array to support AFx operations, adopting a modded RV32 ISA and faster Fused-Multiply-Adder (FMA) units. The end goal is not convention—it's breaking the rules of computing and physics to achieve faster inference, lower memory consumption, and the same accuracy.
